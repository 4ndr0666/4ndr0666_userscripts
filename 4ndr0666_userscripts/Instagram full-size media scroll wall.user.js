...
// @updateURL https://update.greasyfork.org/scripts/396337/Instagram%20full-size%20media%20scroll%20wall.meta.js
// ==/UserScript==
...
(function() {
    ...
    function getQueryHash() {
        ...
        let randomJs = allScripts.find(el => el.src && el.src.match(/miOdM842jTv\.js/));
        randomJs = null;
        ...
        if (randomJs) {
            fetch(randomJs.src)
            .then(resp => {
                console.log('resp 4', resp);
                return resp.text();
            })
            .then(text => {
                doc_id = text.match(/,"polarisUnexpected"\],\(function\(a,b,c,d,e,f,g\)\{"use strict";e=0;f="(\d+)";var h="/)?.[1]; //text.match(/l\.pagination\},queryId:"([a-f0-9]+)"/); //const s="05555555555555555555555555555554",E="
                let app_id = text.match(/instagramWebDesktopFBAppId='(\d+)'/)?.[1];
                app_id = app_id || text.match(/;\s*([a-z])\s*=\s*(['"])(\d+)\2;[^}]*f\.instagramWebDesktopFBAppId\s*=\s*\1;/)?.[3];
                let asbd_id = text.match(/ASBD_ID='(\d+)'/)?.[1];
                asbd_id = asbd_id || text.match(/([a-z]+)="(\d+)";f.ASBD_ID=\1/)?.[2];
                console.log('app_id, asbd_id, doc_id', app_id, asbd_id, doc_id);
                doc_id && notLoaded && loadImages(query_id, query_hash, doc_id, app_id, asbd_id);
            });
        }
    ...
    // https://www.instagram.com/graphql/query/?doc_id=<doc_id>&variables={"id":"<profile_id>","after":"<after_code>","first":12}
    function loadImages(query_id=0, query_hash=0, doc_id=0, app_id=936619743392459, asbd_id=129477, after=null) { // asbd_id=198387
        notLoaded = false;
        console.log('MODE', MODE, 'query_id', query_id, 'query_hash', query_hash);
        ...
        if (MODE == 'profile') {
            ...
            console.log(imageListQueryUrl, queryVariablesString, init);
        }
        if (MODE == 'tagged') {
            if (!userId) {
                console.log("Couldn't find user ID!", userId);
